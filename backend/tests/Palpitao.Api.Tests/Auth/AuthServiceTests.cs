...
public class AuthServiceTests
{
    ...
    private static Palpitao.Api.DTOs.Groups.CreateGroupRequest NewGroup(
        string groupName = "Grupo dos Amigos", string email = "owner@x.com", string password = "Senha123",
        TournamentType tournamentType = TournamentType.PalpitaoEngland) => new()
        {
            GroupName = groupName,
            AdminName = "Dono",
            Email = email,
    ...
    [Fact]
    public async Task CreateGroup_requires_a_tournament_type()
    {
        using var db = CreateContext();
        var request = NewGroup();
        request.TournamentType = null;

        var ex = await Assert.ThrowsAsync<BusinessRuleException>(
            () => Service(db).CreateGroupAsync(request, Ct));
        Assert.Equal("tournamentType.required", ex.Key);
    }

    [Fact]
    public async Task CreateGroup_persists_the_world_cup_type()
    {
        using var db = CreateContext();

        await Service(db).CreateGroupAsync(
            NewGroup(groupName: "Copa 2026", email: "wc@x.com", tournamentType: TournamentType.FifaWorldCup), Ct);

        var group = await db.Groups.SingleAsync(g => g.Slug == "copa-2026", Ct);
        Assert.Equal(TournamentType.FifaWorldCup, group.TournamentType);
    }
