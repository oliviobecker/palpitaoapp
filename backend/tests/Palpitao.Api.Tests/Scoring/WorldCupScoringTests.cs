...
public class WorldCupScoringTests
{
    ...
    private static readonly ScoringRuleSet Rules =
        ScoringDefaults.ForTournamentType(TournamentType.FifaWorldCup, new Dictionary<Guid, Competition>());
    ...
    public void Phase_multipliers(MatchPhase phase, int expected)
        => Assert.Equal(expected, _s.GetMultiplier(Rules, Competition.FifaWorldCup, phase, isClassicPair: false));

    // --- Classics (both world champions) double in the knockout -------------

    [Fact]
    public void Brazil_vs_Germany_group_stage_is_not_doubled()
        => Assert.Equal(1, _s.GetMultiplier(Rules, Competition.FifaWorldCup, MatchPhase.WorldCupGroupStage, isClassicPair: true));

    [Fact]
    public void Brazil_vs_Germany_round_of_32_is_x4()
        => Assert.Equal(4, _s.GetMultiplier(Rules, Competition.FifaWorldCup, MatchPhase.WorldCupRoundOf32, isClassicPair: true));

    [Fact]
    public void Argentina_vs_France_round_of_16_is_x4()
        => Assert.Equal(4, _s.GetMultiplier(Rules, Competition.FifaWorldCup, MatchPhase.WorldCupRoundOf16, isClassicPair: true));

    [Fact]
    public void Brazil_vs_England_quarter_final_is_x6()
        => Assert.Equal(6, _s.GetMultiplier(Rules, Competition.FifaWorldCup, MatchPhase.WorldCupQuarterFinal, isClassicPair: true));

    [Fact]
    public void Spain_vs_Uruguay_final_is_x6()
        => Assert.Equal(6, _s.GetMultiplier(Rules, Competition.FifaWorldCup, MatchPhase.WorldCupFinal, isClassicPair: true));

    [Fact]
    public void Brazil_vs_Japan_quarter_final_is_x3_because_japan_is_not_a_champion()
        => Assert.Equal(3, _s.GetMultiplier(Rules, Competition.FifaWorldCup, MatchPhase.WorldCupQuarterFinal, isClassicPair: false));
    ...
    private int Final(int ph, int pa, int ah, int aa, MatchPhase phase)
        => _s.ScorePrediction(Rules, ph, pa, ah, aa, Competition.FifaWorldCup, phase, isClassicPair: false).FinalPoints;
}
