...
export class AdminMatches implements OnInit {
  ...
  /** Competitions/phases offered depend on the group's certame type. */
  protected readonly competitions = computed(() =>
    competitionsForType(this.group.tournamentType()).map((value) => ({
      value,
      label: AdminMatches.COMPETITION_LABELS[value],
    })),
  );
  protected readonly phases = computed(() => phasesForType(this.group.tournamentType()));
  ...
  protected readonly form = this.fb.nonNullable.group({
    competition: [
      this.group.isWorldCup() ? Competition.FifaWorldCup : Competition.PremierLeague,
      Validators.required,
    ],
    phase: [
      this.group.isWorldCup() ? MatchPhase.WorldCupGroupStage : MatchPhase.Regular,
      Validators.required,
    ],
