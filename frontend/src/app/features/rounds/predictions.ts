...
import { RoundStatus, WORLD_CUP_FLAVIO_PHASES } from '../../core/models/enums';
...
export class Predictions implements OnInit {
  ...
  protected readonly flavioActive = computed(
    () =>
      this.groupContext.isWorldCup() &&
      this.matches().some((m) => WORLD_CUP_FLAVIO_PHASES.includes(m.phase)),
  );
